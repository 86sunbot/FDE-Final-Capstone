...
def test_machine_readable_contracts_are_strict_and_versioned():
    for name in ["event-envelope", "command", "recommendation"]:
        contract = json.loads((ROOT / f"docs/stages/stage_09/contracts/{name}.schema.json").read_text(encoding="utf-8"))
...
def test_recommendation_contract_has_no_authority_or_execution_fields():
    contract = json.loads((ROOT / "docs/stages/stage_09/contracts/recommendation.schema.json").read_text(encoding="utf-8"))
