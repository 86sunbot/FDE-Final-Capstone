...
The existing PowerPoint deck has a stale closing speaker note that overstates this synthetic academic POC as a complete, mathematically verified GxP platform. The [speaker guide](../presentation/SPEAKER_SCRIPT_AND_TIMING_GUIDE_40MIN.md) and [generator](../presentation/generate_presentation.py) now use the correct boundary, but the committed deck itself has not been regenerated in this repository-organization pass. Regenerate and review it before presenting. Its other claims should receive a separate factual review against the current evidence register.
